%Poner Las muestras de Monte carlo en todos 1000 menos  en el secuencial de
%wald que es para aceptacion  y rechazo 500
...
% ICC más realista en este contexto
rho = 0.03;
...
% Número de municipios a seleccionar
nI = ceil(municipios_totales / k_opt);
...
Rb_cong = 1000;
cont_cong = 0;
estPscong = zeros(Rb_cong,1);
Bscong = zeros(Rb_cong,1);

for b = 1:Rb_cong
    %Primera etapa con muestreo proporcional al tamaño
    mun_sel = lahiri(max(x), x, nI);
    ...
    for i = 1:nI
        ...
        firmas_mun = binornd(1, p_municipios(h), Nh, 1);
        % MAS sin reemplazo
        sel = mas(Nh, nh);
        muestra_h = firmas_mun(sel);
        % Estimador del total del conglomerado
        estauyi(i) = Nh * mean(muestra_h);
    end
    [estTau, estvTau] = bippstau2026(pis, nI, estauyi);
...
coste_cong  = 500 + 150*nI + 2 * n_cong;
